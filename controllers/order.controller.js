...
/**
 * @desc    Search orders by email or phone (for tracking without order ID)
 * @route   GET /api/orders/track?email=xxx or ?phone=xxx
 * @access  Public
 */
const searchOrdersForTracking = async (req, res) => {
  try {
    const email = (req.query.email || '').toString().trim().toLowerCase();
    const phone = (req.query.phone || '').toString().trim().replace(/\s+/g, '');
    ...
    const orders = await Order.find(query)
      .sort({ createdAt: -1 })
      .limit(10)
      .select('_id orderStatus paymentStatus total createdAt items');

    if (orders.length === 0) {
      return res.status(404).json({ message: 'No orders found for this email or phone' });
    }
